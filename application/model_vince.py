...
class Connect4KerasModel:
  ...
  def train(self, train_data, train_labels, batch_size):
    self.model.fit(train_data, train_labels, epochs=150, batch_size=batch_size)

if __name__ == '__main__':
  """
  example: python3.6 model_vince.py -w 7 -b 7 -r 0 -t minimax
  ...
  if (args.random > 0):
    with open(f'../data/{args.type}_random_{args.random}.pickle', 'rb') as board_states_file:
      states_from_file = pickle.load(board_states_file)
      states_from_file_winning = list(map(lambda state: state[3], states_from_file))
  ...
  for index in range(args.amount):
    ...
    connect_four_model.model.save(f'../models/{args.type}_t{args.winning + args.blocking + args.random}_w{args.winning}_b{args.blocking}_r{args.random}_model_columnchoice_{index}.h5')